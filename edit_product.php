...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES["image"]["name"])) {
        ...
        $query = "UPDATE products SET name=?, category=?, image=?, description=?, price=?, offer_price=?, quantity=? WHERE id=?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssssddii", $name, $category, $image, $description, $price, $offer_price, $quantity, $id);
    } else {
        $query = "UPDATE products SET name=?, category=?, description=?, price=?, offer_price=?, quantity=? WHERE id=?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("sssddii", $name, $category, $description, $price, $offer_price, $quantity, $id);
    }
